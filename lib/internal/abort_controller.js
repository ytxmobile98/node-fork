...
let _MessageChannel;
let markConstructorTransferable;
...
function lazyMarkConstructorTransferable(C) {
  markConstructorTransferable ??=
    require('internal/worker/js_transferable').markConstructorTransferable;
  return markConstructorTransferable(C);
}
...
const kThing = Symbol('kThing');

class AbortSignal extends EventTarget {
  constructor(aborted, reason, thing) {
    if (thing !== kThing) throw new ERR_ILLEGAL_CONSTRUCTOR();
    super();
...
ObjectDefineProperty(AbortSignal, 'length', { value: 0 });
...
function createAbortSignal(aborted = false, reason = undefined) {
  if (markConstructorTransferable == null) {
    markConstructorTransferable ??=
      require('internal/worker/js_transferable').markConstructorTransferable;
    markConstructorTransferable(AbortSignal)
  }

  return new AbortSignal(aborted, reason, kThing);
}
