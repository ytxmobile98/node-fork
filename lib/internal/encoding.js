...
const kAsciiWhitespaces = new SafeSet([ HT, LF, FF, CR, SPACE ]);

function isAsciiWhitespace(ch) {
  return (typeof ch === 'string') &&
    kAsciiWhitespaces.has(ch.codePointAt(0));
}
...
function trimAsciiWhitespace(label) {
  let s = 0;
  let e = label.length;
  while (s < e && isAsciiWhitespace(label[s])) {
    s++;
  }
  while (e > s && isAsciiWhitespace(label[e - 1])) {
    e--;
  }
